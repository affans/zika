...
#daly(DW = 0.55, A = 40, LD = 20, LY = 25, BA = 40, Dis = 0.0, K=0)
#daly(DW = 0.55, A = 40, LD = 20, LY = 25, BA = 40, Dis = 0.0, K=1)
#daly(DW = 0.55, A = 40, LD = 20, LY = 25, BA = 40, Dis = 0.03, K=0)
#daly(DW = 0.55, A = 40, LD = 20, LY = 25, BA = 40, Dis = 0.03, K=1)


## returns a named list of strings, corresponding to simulation data output
fnames <- function(cn="Colombia"){
  fpath = paste0("/home/affans/zika/", cn, "/")
  ## do a little string processing on the folder names
  a = list.files(fpath)
  stopifnot(length(a) == 2)
  ## check whether "00" exists in a[1] then a[1] is nv
  pr = substr(unlist(strsplit(a[1], "_"))[2], 9, 10)
  if (pr == "00"){
    nv = paste0(fpath, a[1], "/")
    wv = paste0(fpath, a[2], "/")
  } else {
  ...
  #nv = paste0(fpath, "/Asymp100Iso0_Coverage00Pre1_0-2982/")
  #wv = paste0(fpath, "/Asymp100Iso0_Coverage1080Pre1_0-2982/")
  lv = c("wv", "nv")
  mainlist = list()
  mainlist["dir"] = fpath
  for(i in lv){
...
## returns costs for each country
get_country_costs <- function(cn){
  a = fromJSON("country_data.json")
  cl = list()
  for(i in 1:length(a$name)){
    if (a$name[i] == cn){
      cl["symp"] = a$data$cost_symp[i]
      cl["gbs"] = a$data$cost_gbs[i]
      cl["micro"] = a$data$cost_micro[i]
    }
  }
  return(cl)
}
...
process_simulations <- function(cn, fn, vaccineprice=0){
  ...
  ## cost variables
  sympcost = get_country_costs(cn)$symp
  testcost = 150
  gbscost = get_country_costs(cn)$gbs
  microcost = get_country_costs(cn)$micro
...
run_analysis <- function(cn){
  ...
  data.costs <- matrix(0,nrow=2000,ncol=length(vaccineprices)); 
  data.dalys <- matrix(0,nrow=2000,ncol=length(vaccineprices)); 
  
  ## get the base data tables 
  filenames = fnames(cn)
  
  ## process the simulations without vaccine... dont need to put this inside loop
  nv_raw = process_simulations(cn, filenames$nv, vaccineprice=0)
  for(i in 1:length(vaccineprices)){
    #print(i)
    ## WHAT FILES DO YOU WANT TO PROCESS?
    wv_raw = process_simulations(cn, filenames$wv, vaccineprice=vaccineprices[i])
  ...
  icerfilename = paste0(cn, "ICER.dat")
  costsfilename = paste0(cn, "costs.dat")
  dalysfilename = paste0(cn, "dalys.dat")
  ceacfilename = paste0(cn, "CEAC.dat")
  fwrite(data.table(data.ICER), file = icerfilename, col.names = F, row.names = F)
  fwrite(data.table(data.costs), file = costsfilename, col.names = F, row.names = F)
  fwrite(data.table(data.dalys), file = dalysfilename, col.names = F, row.names = F)
...
run_analysis("Boliva")
run_analysis("Brazil")
run_analysis("Colombia")
run_analysis("CostaRica")
run_analysis("Ecuador")
run_analysis("ElSalvador")
run_analysis("FrenchGuiana")
run_analysis("Guatemala")
run_analysis("Guyana")
run_analysis("Honduras")
run_analysis("Mexico")
run_analysis("Nicaragua")
run_analysis("Panama")
run_analysis("Paraguay")
run_analysis("Peru")
run_analysis("Suriname")
run_analysis("Venezuela")
